...
title: "第 4 章 串"
sidebarTitle: "串"
description: "整理串的顺序与链式存储、朴素模式匹配和 KMP 算法。"
tags: ["考研408", "数据结构"]
---

本章整理串这一特殊线性表。复习时除掌握顺序、链式和块链存储的空间利用率差异外，还应重点掌握模式匹配，尤其是 KMP 中 `next` 与 `nextval` 数组的求解和应用。
...
<AccordionGroup>
  ...
  <Accordion title="KMP 算法核心思想：在“省”什么？">
    ...
    **前缀、后缀、最长相等真前后缀 (LPS)**：
    ...
    - **LPS (Longest Proper Prefix which is also Suffix)**：最长相等真前后缀长度。例如 `X = "ababab"`，其 LPS 长度为 `4`（"abab"）
  </Accordion>
